...
# 1.1 Background
The COVID-19 pandemic triggered an abrupt and unprecedented shift toward digital life in Canada. Between 2020 and 2022, daily activities such as work, financial transactions, education, and social engagement moved online at a scale and speed not previously seen. As public health restrictions eased and in-person routines resumed, Canadians were left navigating a hybrid environment, where digital systems remained deeply embedded in everyday life. This rapid digitization not only transformed online habits, but also reshaped how people perceived cyber security, digital risks, and the institutions that manage and protect their data.
...
| Cyber Security                  | Trust                  | Demographics              |
|---------------------------------|------------------------|---------------------------|
| Personal Data Protection        | Trust in Technologies  | Age group                 |
| ID Verification Measures        | Trust in Organizations | Gender                    |
| Cyber security Incidents        |                        | Location density          |
| Financial Loss                  |                        | Family income quintile    |
| Post-Incident Actions           |                        | Highest education         |
| PC/Laptop Protection            |                        | Life satisfaction         |
|                                 |                        | Employment status         |
|                                 |                        | Immigrant status          |
|                                 |                        | Visible minority          |
|                                 |                        | Disability status         |
...
By summarizing categories into single proportion or change scores, we aimed to reduce hyper-granularity while preserving the intensity or breadth of behaviors and experiences. However, some of these derived variables overlap because they use the same source columns to help us consider which data framing works best on the available variables. We chose to work with the demographic variables and numerical scores derived from the cyber security and trust items, rather than the simplified categories or binary variables. The score-based approach allows us to reduce hyper-granularity while still preserving the underlying scale, nuance and proportional information embedded in the original survey responses.
...
# 2.5 Categorical Variables (Demographics)
...
kable(catChiSum(df, varsDemo), digits=4, align = "r") 
...
```{r Categorical Visualizations III, echo=FALSE}
...
# 2.7 Numerical Variables (Scores)
...
```{r Density Plots, echo=FALSE, warning = FALSE}
...
*Model 1*. `trustScore` ~ Demographics
...
### Homoscedacityy: Constant variance of residuals
...
```{r echo=FALSE}
par(mfrow = c(2, 4), mar = c(2.5, 2.5, 2, 1), oma = c(0, 0, 0, 0))
...
## Top Co-Efficients of *Model 1* (`trustScore` ~ Demographics)

```{r Modelling 1, echo=FALSE}

options(scipen=0)

kable(head(getSigCoefs(mod1), 7), digits=3) 
...
## Top Co-Efficients of *Model 2* (`expoScore` ~ Demographics)

```{r Modelling 2, echo=FALSE}

kable(head(getSigCoefs(mod2), 5), digits=3) 
...
## Top Co-Efficients of *Model 3* (`datSecScore` ~ Demographics)

```{r Modelling 3, echo=FALSE}

kable(head(getSigCoefs(mod3), 5), digits=3) 
...
## Top Co-Efficients of *Model 4* (`idSecScore` ~ Demographics)

```{r Modelling 4, echo=FALSE}

kable(head(getSigCoefs(mod4), 5), digits=3) 
```
...
## Top Co-Efficients of *Model 5* (`pcSecScore` ~ Demographics)

```{r Modelling 5, echo=FALSE}

kable(head(getSigCoefs(mod5), 5), digits=3) 
...
## Top Co-Efficients of *Model 6* (`responseScore` ~ Demographics)

```{r Modelling 6, echo=FALSE}

kable(head(getSigCoefs(mod6), 5), digits=3) 
...
## Top Co-Efficients of *Model 7* (`trustScore` ~ Demographics)

```{r Modelling 7, echo=FALSE}

kable(head(getSigCoefs(mod7), 5), digits=3) 
